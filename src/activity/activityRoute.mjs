...
router.get('/getactivity', authenticateCookie, async (req, res) => {
    try {
        ...
        const referralGivenFilter = { referrer_id: userObj._id };
        const referralReceivedFilter = { referee_id: userObj._id };
        const tyftbGivenFilter = { payer_id: userObj._id };
        const tyftbReceivedFilter = { receiver_id: userObj._id };
        const M2MsFilter = {
            $or: [
        ...
        const visitorsFilter = { inviting_member_id: userObj._id };
...
router.get('/getfullactivity', authenticateCookie, async (req, res) => {
    try {
        const userId = req.user && req.user.uid;
        if (!userId) {
            return res.status(400).json({ error: "Missing user id." });
        ...
        if (!filter || filter === 'referral' || filter === 'all') {
            const referralGivenQuery = buildQuery({ referrer_id: userObj._id });
            referral_given = await Referral.countDocuments(referralGivenQuery);

            const referralReceivedQuery = buildQuery({ referee_id: userObj._id });
        ...
        if (!filter || filter === 'm2m' || filter === 'all') {
            const m2mGivenQuery = buildQuery({ member1_id: userObj._id });
            m2m_given = await OneToOneMeeting.countDocuments(m2mGivenQuery);

            const m2mReceivedQuery = buildQuery({ member2_id: userObj._id });
...
router.get('/getactivity-details', authenticateCookie, async (req, res) => {
    try {
        const userId = req.user && req.user.uid;
        if (!userId) {
            return res.status(400).json({ error: "Missing user id." });
        ...
        if (type === 'all' || type === 'm2m') {
            const m2mGivenFilter = { member1_id: userObj._id };
            const m2mReceivedFilter = { member2_id: userObj._id };
        ...
        if (type === 'all') {
            const visitorFilter = { inviting_member_id: userObj._id };
